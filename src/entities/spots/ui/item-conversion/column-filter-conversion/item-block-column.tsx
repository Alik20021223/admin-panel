...
import { TableRowСonversion } from "@entities/spots/types";


interface ItemBlockColumnProps {
    item: Column<TableRowСonversion>;
    dragHandleProps?: Record<string, unknown>;
}

const ItemBlockColumn: React.FC<ItemBlockColumnProps> = ({ item, dragHandleProps }) => {
    const { columnVisibilityConversion, columnPinningConversion, setColumnPinningConversion } = useSpotsTableStore();

    const isVisible = columnVisibilityConversion[item.id] !== false;
    const isPinned = (columnPinningConversion.left ?? []).includes(item.id);

    const togglePin = () => {
        setColumnPinningConversion((prev) => {
